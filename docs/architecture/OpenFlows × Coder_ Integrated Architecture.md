...
pub trait Node: Send + Sync {
    ...
    async fn post(&self, store: &SharedStore, exec_result: Value) -> Result<()>;
}
...
impl CoderProvisioner {
    async fn provision_pair(&self, pair_id: &str, config: &PairConfig) -> Result<PairWorkspace> {
        ...
        self.client.wait_for_workspace_ready(&workspace.id, Duration::from_secs(300)).await?;
...
| **Unauthorized commands** | Hook scripts (bypassable) | Coder template policy + hooks (belt+braces) |
...
| **User attribution** | GitHub PAT per bot (single identity) | Per-user Coder SSO identity |
| **Secret scanning** | Git hooks on push (push rejection) | Coder template policy + git hooks |
...
| **User identity** | Shared GitHub PAT | Per-user Coder SSO identity |
| **Audit** | SharedStore events (volatile) | Coder audit log + SharedStore events |
| **Network isolation** | None | Per-template egress rules |
| **Model governance** | Per-agent .env config | Centralized admin panel |
